...
function inspectIterable(value, level, options, inspectOptions) {
  ...
  for (const el of iter) {
    if (entriesLength < (inspectOptions.iterableLimit ?? 0)) {
      entries.push(
        options.entryHandler(el, level + 1, inspectOptions, next.bind(iter)),
      );
    }
    entriesLength++;
  }
...
function groupEntries(entries, level, value, iterableLimit = 100) {
  ...
  if (
    ...
    const columns = Math.min(
      Math.round(Math.sqrt(2.5 * biasedMax * entriesLength) / biasedMax),
      Math.floor((80 - (level + 1)) / actualMax),
      15,
    );
    if (columns <= 1) {
      return entries;
...
function inspectMap(value, level, inspectOptions) {
  const options = {
    ...
    entryHandler: (entry, level1, inspectOptions1) => {
      const [key, val] = entry;
      return `${inspectValueWithQuotes(key, level1 + 1, inspectOptions1)} => ${
        inspectValueWithQuotes(val, level1 + 1, inspectOptions1)
      }`;
    },
    group: false,
    sort: inspectOptions.sorted,
  ...
  for (const key of stringKeys) {
    if (inspectOptions.getters) {
      ...
      const inspectedValue = error == null
        ? inspectValueWithQuotes(propertyValue, level + 1, inspectOptions)
  ...
  for (const key1 of symbolKeys) {
    ...
    if (inspectOptions.getters) {
      ...
      if (descriptor.get !== undefined && descriptor.set !== undefined) {
        ...
        entries.push(
          `[${maybeQuoteSymbol(key1)}]: ${
            inspectValueWithQuotes(value[key1], level + 1, inspectOptions)
          }`,
        );
      }
    }
