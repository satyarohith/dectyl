...
function inspectArray(value, level1, inspectOptions1) {
    const dim = maybeColor(colors.dim, inspectOptions1);
    const options = {
        typeName: "Array",
        displayName: "",
        delims: [
        ...
        group: inspectOptions1.compact,
        sort: false
    };
    return inspectIterable(value, level1, options, inspectOptions1);
}
function inspectTypedArray(typedArrayName, value, level2, inspectOptions2) {
    ...
    const options = {
        ...
        group: inspectOptions2.compact,
        sort: false
    };
    return inspectIterable(value, level2, options, inspectOptions2);
}
function inspectSet(value, level3, inspectOptions3) {
    const options = {
        typeName: "Set",
        displayName: "Set",
        delims: [
        ...
        group: false,
        sort: inspectOptions3.sorted
    };
    return inspectIterable(value, level3, options, inspectOptions3);
}
function inspectMap(value, level4, inspectOptions4) {
    const options = {
        typeName: "Map",
        displayName: "Map",
        delims: [
        ...
        group: false,
        sort: inspectOptions4.sorted
    };
    return inspectIterable(value, level4, options, inspectOptions4);
}
...
class DeployWorkerHost {
    ...
    constructor(){
        addEventListener("message", (evt1)=>{
            this.#handleMessage(evt1);
        });
    ...
    fetch(input, requestInit) {
        ...
        const id3 = this.#fetchId++;
        const deferred = new Deferred();
        this.#pendingFetches.set(id3, deferred);
        const init1 = {
            body: {
                type: "null"
            },
        ...
        if (typeof input === "string") {
            url = new URL(input);
            init1.url = url.toString();
        } else if (input instanceof URL) {
            url = input;
            init1.url = input.toString();
        ...
        if (requestInit && inputRequest) {
            ...
            if (requestInit.body != null) {
                [init1.body, bodyStream, headers] = parseBodyInit(inputRequest ?? init1.url, requestInit);
            ...
            init1.headers = parseHeaders(defaultHeaders, (headers ?? requestInit.headers) ?? inputRequest.headers);
            init1.signal = this.#watchSignal(requestInit.signal ?? inputRequest.signal);
            for (const key of INIT_PROPS){
                init1[key] = requestInit[key] ?? inputRequest[key];
            }
        } else if (requestInit) {
            let headers;
            if (requestInit.body != null) {
            ...
            init1.headers = parseHeaders(defaultHeaders, headers ?? requestInit.headers);
            init1.signal = this.#watchSignal(requestInit.signal);
            for (const key of INIT_PROPS){
                init1[key] = requestInit[key];
            }
            ...
            init1.headers = parseHeaders(defaultHeaders, inputRequest.headers);
            init1.signal = this.#watchSignal(inputRequest.signal);
            for (const key of INIT_PROPS){
                init1[key] = inputRequest[key];
            }
        } else {
            init1.headers = parseHeaders(defaultHeaders);
        }
        this.#postMessage({
            type: "fetch",
            id: id3,
        ...
        if (bodyStream) {
            this.#streamBody(id3, bodyStream);
        }
